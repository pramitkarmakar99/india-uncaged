...
@forelse($images as $image)<tr><td><strong>{{ Str::limit($image->caption ?: 'Untitled image',55) }}</strong><small>{{ $image->photographer ?: 'Photographer not set' }}</small></td><td>{{ str($image->category)->replace('_',' ')->title() }}</td><td>{{ $image->tour?->name ?: ($image->destination?->name ?: 'General') }}</td><td><span class="status">{{ $image->published ? 'Published' : 'Draft' }}</span></td><td><a href="{{ route('admin.gallery.edit',$image) }}">Edit</a></td></tr>@empty<tr><td colspan="5" class="empty-cell">No gallery images yet.</td></tr>@endforelse
