...
class PublicSiteController extends Controller
{
    ...
    {
        return view('destinations.index',['destinations'=>Destination::where('published',true)->whereNull('archived_at')->withCount(['tours'=>fn($q)=>$q->where('published',true)->whereNull('archived_at')])->orderBy('state_region')->orderBy('name')->get()]);
    }
    ...
    {
        $query=TourDate::with(['tour.destination'])->where('published',true)->where('start_date','>=',today())->whereHas('tour',fn($q)=>$q->where('published',true)->whereNull('archived_at'));
        if($request->filled('destination')) $query->whereHas('tour',fn($q)=>$q->where('destination_id',$request->query('destination')));
        if($request->filled('month')) { $month=(int)$request->query('month'); if($month>=1 && $month<=12) $query->whereMonth('start_date',$month); }
